<?php
  ...
  if (isset($_POST['submit_receipt'])) {
    ...
    try {
      ...
      //テーブルID : sq_detail_tr_sales_management / テーブル名称：営業依頼書・明細T/R_営業管理へ登録
      receipt_cu_sq_detail_tr_sales_management($entrant);
  ...
  function receipt_cu_sq_detail_tr_sales_management($entrant) {
    global $pdo;
    global $today;
    global $sq_no;
    global $sq_line_no;
    ...
    if (!$row) {
      $data['add_date'] = $today;
      $sql1 = "INSERT INTO sq_detail_tr_sales_management (sq_no, sq_line_no, entrant, reception, reception_date, add_date) 
            VALUES(:sq_no, :sq_line_no, :entrant, :reception, :reception_date, :add_date)";
    } else {
      $data['upd_date'] = $today;
      $sql1 = "UPDATE sq_detail_tr_sales_management SET entrant=:entrant, reception=:reception, reception_date=:reception_date, upd_date=:upd_date 
  ...
  function entrant_cu_sq_detail_tr_sales_management($entrant_comments, $confirmer_comments, $approver_comments, $mail_to1, $mail_to2, $mail_to3, $mail_to4, $mail_to5, $title) {
    ...
    if ($title == 'sm_entrant') {
      ...
      $sql1 = "UPDATE sq_detail_tr_sales_management SET entrant_comments=:entrant_comments, entrant_date=:entrant_date, mail_to1=:mail_to1, 
    ...
    if ($title == 'sm_approve') {
      ...
      $sql1 = "UPDATE sq_detail_tr_sales_management SET approver=:approver, approver_comments=:approver_comments, approve_date=:approve_date, mail_to1=:mail_to1, 
          mail_to2=:mail_to2, mail_to3=:mail_to3, mail_to4=:mail_to4, mail_to5=:mail_to5, upd_date=:upd_date 
          WHERE sq_no=:sq_no AND sq_line_no=:sq_line_no";
    }
