...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $(".updateBtn").click(function(){
      ...
      $("#input2").attr("action", "sq_detail_tr_sales_management_input3.php?line="+selected+"&title=<?= $title ?>");
    });

    //戻るボタンを押下する場合
    $(".returnBtn").click(function(){
      $("#input2").attr("action", "sq_detail_tr_sales_management_input1.php?title=<?= $title ?>");
    })
  });
  ...
  function saveFormData() {
    ...
    localStorage.setItem('detail_sales_management', jsonData);
  }

  //localStorageからフォームデータをセットする
  const formData = JSON.parse(localStorage.getItem('detail_sales_management'));
  if (formData) {
