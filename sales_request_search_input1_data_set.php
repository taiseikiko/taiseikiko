  ...
  if (isset($_POST['isReturn'])) {
    ...
    if (isset($sq_datas) && !empty($sq_datas)) {
      foreach ($sq_datas as $item) {
        ...
        echo '<td style="text-align:center"><button type="submit" class="updateBtn" data-sq_no="' . htmlspecialchars($item['sq_no']) . '" 
        data-sq_line_no="' . htmlspecialchars($item['sq_line_no']) . '" name="process2" value="detail">詳細</button></td>';
