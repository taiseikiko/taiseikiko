<?php
  ...
  if (isset($_POST['submit_receipt'])) {
    ...
    try {
      ...
      //テーブルID : sq_detail_tr_procurement / テーブル名称：営業依頼書・明細T/R_営業管理へ登録
      receipt_cu_sq_detail_tr_procurement($entrant);
    ...
    if ($success) {
      echo "<script>
        window.close();
        window.opener.location.href='sq_detail_tr_procurement_input1.php?title=pc_receipt';
      </script>";
    }
  }
  ...
  if (isset($_POST['submit_entrant'])) {
    ...
    try {
      $pdo->beginTransaction();
      //テーブルID : sq_detail_tr_procurement / テーブル名称：営業依頼書・明細T/R_営業管理
      entrant_cu_sq_detail_tr_procurement($entrant_comments, $confirmer_comments, $approver_comments, $mail_to1, $mail_to2, $mail_to3, $mail_to4, $mail_to5, $title);
    ...
    if ($success) {
      echo "<script>
        window.location.href='sq_detail_tr_procurement_input1.php?title=".$title."';
      </script>";
    }
  }
  ...
  function receipt_cu_sq_detail_tr_procurement($entrant) {
    global $pdo;
    global $today;
    global $sq_no;
    global $sq_line_no;
    ...
    if (!$row) {
      $data['add_date'] = $today;
      $sql1 = "INSERT INTO sq_detail_tr_procurement (sq_no, sq_line_no, entrant, reception, reception_date, add_date) 
            VALUES(:sq_no, :sq_line_no, :entrant, :reception, :reception_date, :add_date)";
    } else {
      $data['upd_date'] = $today;
      $sql1 = "UPDATE sq_detail_tr_procurement SET entrant=:entrant, reception=:reception, reception_date=:reception_date, upd_date=:upd_date 
  ...
  /*----------------------------------------------------------------入力画面の関数開始------------------------------------------------------------------------------------- */
  //テーブルID : sq_detail_tr_procurement / テーブル名称：営業依頼書・明細T/R_営業管理
  function entrant_cu_sq_detail_tr_procurement($entrant_comments, $confirmer_comments, $approver_comments, $mail_to1, $mail_to2, $mail_to3, $mail_to4, $mail_to5, $title) {
    global $pdo;
    global $today;
    global $sq_no;
    global $sq_line_no;
    ...
    if ($title == 'pc_entrant') {
      ...
      $sql1 = "UPDATE sq_detail_tr_procurement SET entrant_comments=:entrant_comments, entrant_date=:entrant_date, mail_to1=:mail_to1, 
    ...
    if ($title == 'pc_confirm') {
      ...
      $sql1 = "UPDATE sq_detail_tr_procurement SET confirmer=:confirmer, confirmer_comments=:confirmer_comments, confirm_date=:confirm_date, mail_to1=:mail_to1, 
          mail_to2=:mail_to2, mail_to3=:mail_to3, mail_to4=:mail_to4, mail_to5=:mail_to5, upd_date=:upd_date 
          WHERE sq_no=:sq_no AND sq_line_no=:sq_line_no";
    }
    ...
    if ($title == 'pc_approve') {
      ...
      $sql1 = "UPDATE sq_detail_tr_procurement SET approver=:approver, approver_comments=:approver_comments, approve_date=:approve_date, mail_to1=:mail_to1, 
          mail_to2=:mail_to2, mail_to3=:mail_to3, mail_to4=:mail_to4, mail_to5=:mail_to5, upd_date=:upd_date 
          WHERE sq_no=:sq_no AND sq_line_no=:sq_line_no";
    }
    ...
    if ($row) {
      for ($i = 1; $i <= 5; $i++) {
        if ($row['route'.$i.'_dept'] == $dept_id) {
          ...
          } else if ($title == 'pc_confirm') {
            $datas['route'.$i.'_confirm_date'] = $today;
          } else if ($title == 'pc_approve') {
